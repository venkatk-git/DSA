...
public  class DecodeWays{
    ...
    private static int numDecodings(String s, HashMap<String, Integer> memo){
        ...
        int takeOne = numDecodings(s.substring(1), memo);
        int takeTwo = 0;

        if(s.length() > 1){
            int num = Integer.parseInt(s.substring(0, 2));
            if(num > 0 && num < 27)
                takeTwo = numDecodings(s.substring(2), memo);
        }
